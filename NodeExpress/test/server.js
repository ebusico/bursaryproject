...
describe('trainee/', () => {
	it('Should get a staus of 200', (done) => {
		chai.request(server).get('/trainee/').end((err, res) => {
...
describe('/add trainee', () => {
	it('Should add a trainee account',  (done) => {
		...
		chai.request(server).post('/trainee/add').send(addUser).end((err, res) => {
...
describe('/update:id', () => {
	it('Should update user account',  (done) => {
		...
		chai.request(server).post('/trainee/update/${id}').send(updateUser).end((err, res) => {
			res.status.should.be.equal(200);
...
describe('/trainee:id', () => {
	it('Should return details of "John"', (done) => {
		var id = '5cae04f9f631d12a18c6b3e4';
		chai.request(server).get('/trainee/${id}').end((err, res) => {
			res.should.have.status(200);
			res.body.should.be.a('array');
			done();
		});
...
describe('/update-password', () => {
	it('should update the trainee password', (done) => {
		var id = '5cae04f9f631d12a18c6b3e4';
		chai.request(server).post('/trainee/update-password/${id}').send().end((err, res) => {
			res.status.should.be.equal(200);
...
describe('/POST Register', function () {
	it('Should Register user account', function (done) {
		chai.request(server).post('/trainee/register').send(register_details).end((err, res) => {
			res.status.should.be.equal(200);
	...
	it('should give status 400 when incorrect password is used', function (done){
		chai.request(server).post('/trainee/login').field('username', 'iqMNUogGzQnbsqLwjeu61DDdsN9rFCPnSCusBfg1whc=')
